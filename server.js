...
function isValidChinaPhone(contact) {
...
async function route(req, res) {
  ...
  if (req.method === "POST" && url.pathname === "/api/member/login") {
    ...
    if (!name || !contact) {
      sendJson(res, 400, { message: "请填写会员昵称和手机号" });
      return;
    }
    if (!isValidChinaPhone(contact)) {
      sendJson(res, 400, { message: "请填写正确的 11 位手机号" });
      return;
    }
  ...
  if (req.method === "POST" && url.pathname === "/api/orders") {
    ...
    let order = clientOrderId
      ? db.orders.find((item) => (item.id === clientOrderId || item.clientOrderId === clientOrderId) && item.memberId === member.id)
      : null;
    if (!order) {
      order = {
